...
function createTableChart_(sheet, chartTitle, row, col, width, height) {
  ...
  const seriesCount = effectiveChartColCount - 1;
  const chart = sheet.newChart()
    ...
    .setOption("legend", { position: effectiveChartColCount > 2 ? "right" : "none", textStyle: { fontSize: 10 } })
    .setOption("series", summaryDataLabelSeriesOptions_(seriesCount))
    .setOption("annotations", { alwaysOutside: true, textStyle: { fontSize: 9, color: "#3C4043" } })
...
function summaryDataLabelSeriesOptions_(seriesCount) {
  const options = {};
  for (let index = 0; index < seriesCount; index++) {
    options[index] = {
      ...
      annotations: {
        textStyle: { fontSize: 9, color: "#3C4043" },
      },
    };
  }
  return options;
...
function resetChartDataSheet_(ss) {
  const sheet = ss.getSheetByName("_ChartData") || ss.insertSheet("_ChartData");
  sheet.clear();
  sheet.hideSheet();
  return sheet;
}

function getChartDataSheet_(ss) {
  const sheet = ss.getSheetByName("_ChartData") || ss.insertSheet("_ChartData");
  sheet.hideSheet();
  return sheet;
}

function buildWowChartRange_(ss, sourceSheetName) {
  ...
  const displayValues = source.getRange(1, 1, lastRow, lastCol).getDisplayValues();
  const rawValues = source.getRange(1, 1, lastRow, lastCol).getValues();
  const headers = displayValues[0];

  const isHourly = sourceSheetName === "HourlySummary";
  let numCols;
  if (isHourly) {
    numCols = 2;
  } else {
    const totalIndex = headers.indexOf("Jumlah Keseluruhan");
    numCols = totalIndex > -1 ? totalIndex : lastCol;
  }
  ...
  for (let r = 0; r < lastRow; r++) {
    ...
    for (let c = 0; c < numCols; c++) {
      if (c === 0 || r === 0) {
        row.push(displayValues[r][c]);
      } else {
        row.push(Number(rawValues[r][c]) || 0);
      }
    }
    output.push(row);
  }

  if (output.length < 2 || output[0].length < 2) return null;

  const chartDataSheet = getChartDataSheet_(ss);
  const startRow = chartDataSheet.getLastRow() + 2;
  chartDataSheet.getRange(startRow, 1, output.length, output[0].length).setValues(output);
  chartDataSheet.getRange(startRow + 1, 2, output.length - 1, output[0].length - 1).setNumberFormat("0");

  return {
    range: chartDataSheet.getRange(startRow, 1, output.length, output[0].length),
    labels: output[0].slice(1),
    seriesCount: output[0].length - 1,
    isHourly: isHourly,
  };
}
...
function createWowColumnChart_(ss, dashboardSheet, sourceSheetName, row, col, title) {
  const chartData = buildWowChartRange_(ss, sourceSheetName);
  if (!chartData) return;

  const chart = dashboardSheet.newChart()
    .setChartType(Charts.ChartType.COLUMN)
    .addRange(chartData.range)
    .setPosition(row, col, 0, 0)
    ...
    .setOption("series", summaryDataLabelSeriesOptions_(chartData.seriesCount))
    .setOption("annotations", { alwaysOutside: true, textStyle: { fontSize: 9, color: "#3C4043" } })
    .setOption("hAxis", {
      textStyle: { color: "#3C4043" },
      titleTextStyle: { color: "#3C4043" },
  ...
  createManualLegend_(dashboardSheet, row, 11, chartData.labels, summaryChartColors_());
}

function createManualLegend_(sheet, row, col, labels, colors) {
  const titleRange = sheet.getRange(row, col, 1, 4);
  titleRange.merge()
    .setValue("PETUNJUK GRAF")
    .setBackground("#E8F0FE")
    .setFontColor("#174EA6")
    .setFontWeight("bold")
    .setHorizontalAlignment("center");
